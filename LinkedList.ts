...
export class LinkedList implements INodeList {
    ...
    public prepend(value: number | string | {}): LinkedList {
        // Создаём новый узел, который будет новым head, при создании передаем второй аргумент, который указывает что его "next" будет текущий head,
    ...
    public append(value: number | string | {}): LinkedList {
        ...
        // Присоединяем новый узел к концу связного списка. Берём последний узел и указываем, что его next будет новым узлом.
    ...
    public delete(value: number | string | {}): LinkedListNode | null {
        ...
        // Если следующий узел должен быть удален, делаем узел через один, следующим для проверки.
        ...
        // Проверяем, должен ли tail быть удален. Так как, если в цикле мы удаляем последний узел, то с предпоследнего узла убираем только ссылку на него.
    ...
    public deleteTail(): LinkedListNode | null {
        ...
        // Если в связном списке много узлов. Перебираем все узлы и находим предпоследний узел, убираем ссылку «next» на последний узел.
        ...
        // В данном случае currentNode - это предпоследний узел или head, который становится последним узлом.
    ...
    public deleteHead(): LinkedListNode | null {
        ...
        // Если у head есть ссылка на следующий "next" узел то делаем его новым head.

        if (this.head.next) {
            this.head = this.head.next;
        } else {
            // Если у head нет ссылки на следующий "next" узел, то мы удаляем последний узел.
...
// Создаём массив из всех узлов
    ...
    public toString(callback?: Fn): string {
        ...
        return (
            this.toArray()
                // На каждом узле вызываем метод toString что бы получить значение в виде строки.
...
// Обратный список
    public reverse(): LinkedList {
        ...
        while (currNode) {
            ...
            // Меняем ссылку на следующий "next" узел текущего узла, чтобы он ссылался на предыдущий узел.
            // Так как мы меняем их местами, нужно поменять и ссылки на узлы.
            // Изначально, первый узел не имеет предыдущего узла, поэтому после перестановки его "next" станет "null".

            currNode.next = prevNode;

            // Перемещаем узлы prevNode и currNode на один шаг вперед. Текущий узел делаем предыдущим.
        ...
        // В данном случае prevNode это последний узел, поэтому, после reverse, он становится первым.
